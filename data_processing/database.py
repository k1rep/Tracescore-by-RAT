...
def read_tracescore(path):
    ...
    cursor.execute(
        ...
        # "select issue_id, type, resolved_date, summary, description, created_date from issue order by resolved_date")
    result = cursor.fetchall()
    ...
    # # read files
    cursor.execute("select issue_id, commit_hash, file_path, committed_date from v_code_change where file_path like '%.java' and issue_id in (select issue_id from issue where fixed_date is not null)")
                   # "SELECT issue_id, code_change.commit_hash, file_path, change_set.committed_date FROM code_change JOIN change_set_link ON code_change.commit_hash = change_set_link.commit_hash JOIN change_set ON change_set.commit_hash = change_set_link.commit_hash")
    ...
    connection.text_factory = str
    cursor = connection.cursor()
    cursor.execute("select * from code_change")
    result = cursor.fetchall()
    for tmp in result:
