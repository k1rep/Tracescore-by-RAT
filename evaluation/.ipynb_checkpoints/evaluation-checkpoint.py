...
def getAvPrecision(true_list, pred_list):
    count_positive_in_top_j = 0

    ap_sum = 0
    for j, j_filename in enumerate(pred_list):
        if j_filename in true_list:
            count_positive_in_top_j += 1
            ap_sum += (count_positive_in_top_j / (j+1)) / len(true_list)
    return ap_sum
